...
function getMaxLifeValue() {
  const enterValue = prompt("플레이어와 몬스터의 최대 체력을 정해주세요.", "100");
...
try {
  ...
  console.log(error);
  chosenMaxLife = 100;
  alert("숫자를 입력하지 않아, 기본값인 100으로 초기화 되었습니다.");
} 
...
function endRound() {
  ...
  if (currentPlayerHealth <= 0 && hasBonusLife) {
    ...
    alert("bonusLife를 하나 사용하여 부활하셨습니다.");
  }
  ...
  if (currentMonsterHealth <= 0 && currentPlayerHealth > 0) {
    ...
    alert("Draw");
    writeToLog(LOG_EVENT_GAME_OVER, "A_DRAW", currentMonsterHealth, currentPlayerHealth);
  }
...
function attackMonster(mode) {
  const maxDamage = mode === MODE_ATTACK ? ATTACK_VALUE : STRONG_ATTACK_VAULE;
...
function healPlayerHandler() {
  ...
  if (currentPlayerHealth >= chosenMaxLife - HEAL_VALUE) {
    alert("초기 최대 체력 이상으로 회복할 수 없습니다.");
